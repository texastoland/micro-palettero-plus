...
function paletteroCommand(bp)
  ...
  local showMenuCmd = string.format(
    "bash -c \"%s| tr '#' '\t' | fzf --ansi -x -d '\t' --nth ..2 --with-nth 1 --layout=reverse --preview 'echo {2}' --preview-window=up:2:noborder:wrap --info=inline --margin=2,2 --padding=1 --no-multi --cycle --prompt='> ' --header='' \"",
    cat())
